...
def current_dep(pos, vel, xx, yy, q=1, init=False, old_pos=None, old_vel=None):
    ...
    if old_pos is not None:
        xidx_old, yidx_old, Sx_old, Sy_old = particle_shape(old_pos, x, y, q)
    else:
...
def jfunc_dep(
    ...
    if np.isclose(vy, 0):
        xsign = vx / abs(vx)
        dist = xsign*(wall_distx - xsign*x0) / cos(theta)
        pml_dist = xsign*(Lx - xsign*x0) / cos(theta)
    elif np.isclose(vx, 0):
        ysign = vy / abs(vy)
        dist = ysign*(wall_disty - ysign * y0) / sin(theta)
        pml_dist = ysign*(Ly - ysign * y0) / sin(theta)
    else:
        ysign = vy / abs(vy)
        xsign = vx / abs(vx)
        dist = min(
            (
                xsign*(wall_distx - xsign * x0) / cos(theta),
                ysign*(wall_disty - ysign * y0) / sin(theta),
            )
        )
        pml_dist = min(
            (
                xsign*(Lx - xsign * x0) / cos(theta),
                ysign*(Ly - ysign * y0) / sin(theta),
            )
        )
    dt = float(0.98 * dx / sqrt(2))
    jtmax = dist / v
    ...
    poslist = [
        torch.tensor([[x0 + v[0, 0] * tp, y0 + v[0, 1] * tp]])
        for (tp, v) in zip(t, vel_list)
    ]
    ...
    Jlist = [
        current_dep(
            poslist[0], 0 * vel_list[0], xx[0, ...], yy[0, ...], init=True, q=q
        ),
        current_dep(
            poslist[0],
            vel_list[0],
    ...
    if smooth:
        Jx = torch.squeeze(
            torch.nn.functional.conv2d(
                torch.unsqueeze(Jx, dim=1), filter, padding="same"
            )
        )
        Jy = torch.squeeze(
            torch.nn.functional.conv2d(
                torch.unsqueeze(Jy, dim=1), filter, padding="same"
            )
        )
        Jz = torch.squeeze(
            torch.nn.functional.conv2d(
                torch.unsqueeze(Jz, dim=1), filter, padding="same"
            )
        )
    Jx *= c_weight
    Jy *= c_weight
